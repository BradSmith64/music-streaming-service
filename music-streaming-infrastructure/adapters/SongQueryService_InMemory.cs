...
public class SongQueryService_InMemory : ISongQueryService
{
    public async Task<List<SongMetadata>> GetSongsAsync(int userId)
    {
        ...
        return songs.Select( entity =>
            new SongMetadata { SongId = entity.Id, Title = entity.Title, AlbumTitle = entity.AlbumTitle, ReleaseDate = entity.ReleaseDate, FileName = entity.FileName, Url = null, LikeCount = entity.Likes.Count, LikedByUser = entity.Likes.Any( ( like ) => like.UserId == userId ) })
        .ToList();
    }
}